...
async def main():
    ...
    async with pool.acquire() as conn:
        await conn.execute("DELETE FROM workspaces WHERE name = $1", WORKSPACE)
        await conn.execute("INSERT INTO workspaces (name) VALUES ($1)", WORKSPACE)

    # Ensure clean state in case a prior run failed mid-test.
    await delete_entities(["test_node_a", "test_node_b", "test_orphan"], workspace=WORKSPACE)
    ...
    print("open_nodes...")
    result = await open_nodes(["test_node_a", "test_node_b", "nonexistent"], workspace=WORKSPACE)
    ...
    result = await get_nodes_by_type("test", workspace=WORKSPACE)
    assert len(result) >= 2
    print(f"  found {len(result)} nodes of type 'test'")

    print("set_summary...")
    result = await set_summary("test_node_a", "A test node for smoke testing", workspace=WORKSPACE)
    assert result["summary"] == "A test node for smoke testing"
    print(f"  summary set: {result['summary']!r}")

    print("set_tags...")
    result = await set_tags("test_node_a", ["smoke", "test", "v2"], workspace=WORKSPACE)
    ...
    result = await replace_observation("test_node_a", 0, "replaced first obs", workspace=WORKSPACE)
    assert result["new_content"] == "replaced first obs"
    print(f"  replaced ordinal 0: {result['new_content']!r}")

    print("delete_observations...")
    result = await delete_observations([{"entity_name": "test_node_a", "ordinals": [1, 99]}], workspace=WORKSPACE)
    ...
    result = await get_relations_between("test_node_a", "test_node_b", workspace=WORKSPACE)
    ...
    result = await get_neighborhood("test_node_a", depth=1, workspace=WORKSPACE)
    names = {n["name"] for n in result["nodes"]}
    assert "test_node_a" in names and "test_node_b" in names, names
    print(f"  nodes in neighborhood: {sorted(names)}")

    print("get_path...")
    result = await get_path("test_node_a", "test_node_b", workspace=WORKSPACE)
    assert result["found"] is True
    assert result["path"] == ["test_node_a", "test_node_b"]
    print(f"  path: {result['path']}")

    result = await get_path("test_node_a", "nonexistent", workspace=WORKSPACE)
    ...
    await create_entities([{"name": "test_orphan", "entity_type": "test"}], workspace=WORKSPACE)
    result = await get_orphans(workspace=WORKSPACE)
    ...
    await add_observations([{"entity_name": "test_orphan", "contents": ["This references test_node_b directly"]}], workspace=WORKSPACE)
    result = await get_relation_gaps(workspace=WORKSPACE)
    ...
    # --- consolidation ---
    print("get_stats...")
    result = await get_stats(workspace=WORKSPACE)
    ...
    result = await get_pending_consolidation(workspace=WORKSPACE)
    ...
    result = await delete_entities(["test_node_a", "test_node_b", "test_orphan", "nonexistent"], workspace=WORKSPACE)
